...
# Simple auth (replace with real users in production)
USERS = {
    "emil@example.com": "password123"  # Add real users
}
...
def login():
    ...
    if st.button("Login"):
        if email in USERS and USERS[email] == password:
            ...
            st.session_state['subscription'] = "Premium"  # Based on user
...
if 'logged_in' not in st.session_state:
    ...
    if st.session_state.get('subscription') == "Premium":
        st.subheader("Premium Features")
        st.write("Advanced forecasts, notifications, full history export coming soon.")
    else:
        st.info("Subscribe for premium features: advanced forecasts, notifications, export.")
